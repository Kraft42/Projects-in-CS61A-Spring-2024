    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                place.ant,self.ant_contained = self,place.ant
            else:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while behind is not None:
            ...
            if ant is not None:
                if ant.is_container:
                    ant.double()
                    ant_c = ant.ant_contained
                    if ant_c is not FireAnt and ant_c is not None:
                        ant_c.double()
                else:
                    if ant is not FireAnt:
                        ant.double()
            behind = behind.exit
        # END Problem 12
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ThrowerAnt.reduce_health(self,amount)
        if self.health <= 0:
            ants_lose()
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while next is not None:
            if next.ant is not None: # Ant
                if next.ant.is_container:
                    result[next.ant] = distance
                    if next.ant.ant_contained is not None and next.ant.ant_contained is not self:
                        result[next.ant.ant_contained] = distance
                elif next.ant is not self:
                    result[next.ant] = distance
            if next.bees is not None: # Bee
                for bee in next.bees:
                    result[bee] = distance
            distance += 1
            next = next.entrance
        return result
