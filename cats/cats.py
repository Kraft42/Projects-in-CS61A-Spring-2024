...
def feline_fixes(typed, source, limit):
    ...
    if typed == source:
        ...
        first_fix = feline_fixes(typed[0],source[0],limit)
        rest_typed,rest_source = typed[1:],source[1:]
        return first_fix + feline_fixes(rest_typed,rest_source,limit - first_fix)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    add = minimum_mewtations(typed[0:], source[1:], limit - 1) + 1 if typed[0] != source[0] else minimum_mewtations(typed[1:], source[1:], limit) 
    remove = minimum_mewtations(typed[1:], source[0:], limit - 1) + 1 if typed[0] != source[0] else minimum_mewtations(typed[1:], source[1:], limit)
    substitute = minimum_mewtations(typed[1:], source[1:], limit - 1) + 1 if typed[0] != source[0] else minimum_mewtations(typed[1:], source[1:], limit)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    cur_progress = corr_cnt/len(source)

    upload({'id':user_id,'progress':cur_progress})
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i]) - 1):
            each_times += [timestamps_per_player[i][j+1] - timestamps_per_player[i][j]]
        all_times += [each_times]
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            cur_check = time(match,j,i)
            if cur_check < min_time:
                min_time = cur_check
                fast_player_index = j
        fast_list += [fast_player_index]

    result = []
    for m in player_indices:
        each_player = []
        for n in range(len(fast_list)):
            if m == fast_list[n]:
                each_player += [get_word(match,n)] 
        result += [each_player]       
